...
(require "private/primitives.rkt"
         "../signature/signature.rkt"
         "../signature/signature-syntax.rkt")
...
(define-syntax define-record*
  (λ (x)
    (syntax-case x ()
      [(_ ?stx ?type-spec ?constructor ?predicate (?accessor ?field-signature) ...)
       (with-syntax*
           ([(?type-name ?type-params ...) (if (identifier? #'?type-spec)
                                               #'(?type-spec)
                                               #'?type-spec)]
            ...
            [real-constructor
             ...
             (let
               ...
               (stepper-syntax-property
                ...
                name))]
            [constructor-proc
             (syntax-property #'(λ (?accessor ...)
                                  (raw-constructor ?accessor ... #f))
                              'inferred-name
                              (syntax-e #'?constructor))]
            [predicate-proc
             (syntax-property #'(λ (thing)
                                  (raw-predicate thing))
                              'inferred-name
                              (syntax-e #'?predicate))]
            [(raw-accessor-proc ...)
             (map-with-index (λ (i _)
                               #`(λ (r)
                 ...
                 (let ()
                   ...
                   (make-info (λ ()
                                (list #f
                                      ...
                                      (map (λ (_) #f) (syntax->list #'(our-accessor ...)))
                                      #f)))))]
            ...
            [signature-def
             (with-syntax*
                 ([(?param ...) (generate-temporaries #'(?accessor ...))]
                  [(component-signature ...)
                   (map (λ (accessor param)
                          (with-syntax
                              ([?accessor accessor]
                               [?param param])
